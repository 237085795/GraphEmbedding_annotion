...
# from test.test import create__file
...
class RandomWalker:
    ...
    def deepwalk_walk(self, walk_length, start_node):
        """返回初始节点为start_node的一条随机游走路径

        :param walk_length:
        :param start_node:
        :return:初始节点为start_node的一条随机游走路径
        """
        walk = [start_node]
        ...
        while len(walk) < walk_length:
            ...
            if len(cur_nbrs) > 0:  # 初始节点没有邻居，随机游走路径就是该初始节点
                walk.append(random.choice(cur_nbrs))  # append往list的最后加元素
            else:
                break
        return walk
    ...
    def simulate_walks(self, num_walks, walk_length, workers=1, verbose=0):
        ...
        results = Parallel(n_jobs=workers, verbose=verbose, )(
            ...
            partition_num(num_walks, workers))  # 并行 out:[[[],[],[],[],[]]] (1, 192400)
        # print("result:")
        # print(np.array(results).shape)
        # create__file("../test/test.txt",''.join('%s' %id for id in results))
        # print(results)
        walks = list(itertools.chain(*results))  # out:[[],[],[],[],[]] (192400,)
        # print("walk:")
        # print(walks)
        # print(np.array(walks).shape)
        # create__file("../test/test1.txt", ''.join('%s' %id for id in walks))
        return walks

    def _simulate_walks(self, nodes, num_walks, walk_length,):
        """对所有nodes做num_walks次随机游走

        :param nodes:
        :param num_walks: 随机游走次数
        :param walk_length: 随机游走长度
        :return:len(nodes)*num_walks个随机游走路径
        """
        walks = []
